...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    n_samples, n_features = X.shape[0], X.shape[-1] # MARK
    # Unpack parameters
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.n_features_in_ = X.shape[-1] # MARK 把下面check结构的注释了，且新加此行
        ...
        for gen in range(prior_generations, self.generations):
            ...
            population = Parallel(n_jobs=n_jobs, # MARK
...
class SymbolicRegressor(BaseSymbolic, RegressorMixin):
    ...
    def predict(self, X):
        ...
        # X = check_array(X) # MARKK
        _, n_features = X.shape[0], X.shape[-1] # MARK        
