...
def _delayy(x1):
    if x1.ndim != 1:
        return np.full_like(x1, np.nan)
    shifted = np.concatenate([[np.nan], x1[:-1]])
    return np.nan_to_num(shifted, nan=0)

def _delta(x1):
    if x1.ndim != 1:
        return np.full_like(x1, np.nan)
    cleaned = np.nan_to_num(x1, nan=0)
    delayed = _delayy(cleaned)
    return cleaned - delayed

def _signedpower(x1):
    if x1.ndim != 1:
        return np.full_like(x1, np.nan)
    cleaned = np.nan_to_num(x1, nan=0)
    return np.sign(cleaned) * (np.abs(cleaned) ** 2)

def _decay_linear(x1):
    if x1.ndim != 1:
        return np.full_like(x1, np.nan)
    df = pd.Series(x1).fillna(0)
    rolling_mean = df.rolling(10, min_periods=1).mean()
    return np.nan_to_num((rolling_mean - df).values, nan=0)

def _stdd(x1):
    if x1.ndim != 1:
        return np.full_like(x1, np.nan)
    std_val = np.std(x1)
    return np.full_like(x1, std_val)

def _rankk(x1):
    if x1.ndim != 1:
        return np.full_like(x1, np.nan)
    return x1.argsort().argsort()  # 0-based升序排名
