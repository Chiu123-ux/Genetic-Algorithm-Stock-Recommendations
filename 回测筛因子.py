...
if __name__ == '__main__':
    ...
    # 回测环境（适应度函数）
    comm = [0.0003, 0.0013] # 买卖费率
    bt = SymbolicTestor(train_data, transact_base='Open',commissions=(comm[0],comm[1])) # 加载数据，根据Close成交,comm是买-卖

    # 生成因子
    factor_num = 1 # 因子编号
    my_cmodel_gp = my_gplearn(function_set, score_func_basic,  feature_names=feature_names, pop_num=100, gen_num=5, random_state=2) # 可以通过换random_state来生成不同因子
