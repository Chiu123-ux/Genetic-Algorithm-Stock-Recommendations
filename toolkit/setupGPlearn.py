...
def gp_save_factor(my_cmodel_gp, factor_num=''):
    ...
    target_path = f"./result/factor/factor{factor_num}.gv"
    if os.path.exists(target_path):
        os.remove(target_path)  # 先删除已存在的文件
    os.rename("./result/Source.gv", target_path)
    #os.rename("./result/Source.gv", f"./result/factor/factor{factor_num}.gv")
    target_path2 = f"./result/factor/factor{factor_num}.pdf"
    if os.path.exists(target_path2):
        os.remove(target_path2)  # 先删除已存在的文件
    os.rename("./result/Source.gv.pdf", target_path2)
...
def my_gplearn(function_set, score_func_basic, pop_num=100, gen_num=3, tour_num=10, random_state = 42, feature_names=None,verbose=1):
    # pop_num, gen_num, tour_num的几个可选值：500, 5, 50; 1000, 3, 20; 1000, 15, 100    ###新加入verbose=1，源代码没有
