...
# 2. 回测函数（支持 Top/Bottom 选股 + 调仓时打印买入股票 + 月末最后交易日调仓）
def backtest_factor(factor_df, price_df, top_n=10, bottom_n=0, initial_capital=1e6, commission=0.0013):
    """每月调仓回测，含数据对齐+选股逻辑+调仓时打印买入股票+月末最后交易日调仓"""
    # 数据对齐（前向填充）
    factor_df = factor_df.reindex_like(price_df).dropna(how='all')
    price_df.ffill(inplace=True)  
    
    # 调仓日：月末最后一个交易日（用 'ME' 替代 'M'，避免周末无交易导致调仓日缺失）
    rebalance_dates = factor_df.resample('ME').last().index.tolist()  # 核心修改点
...
if __name__ == '__main__':
    ...
    # ---------- 6. 测试集：IC判断+回测 ----------
    # 测试集5日收益（用于IC计算）
    ret5_test = np.log(price_test.shift(-5) / price_test)
    ...
    (equity_curve / equity_curve.iloc[0]).plot(title='因子策略净值曲线（每月调仓，月末最后交易日）')
    plt.xlabel('交易日期')
    plt.ylabel('净值')
    plt.grid(True)
    plt.show()